...
pub fn text_input(name: &str, placeholder: &str, value: &str) -> Markup {
    html! {
        input.outline-none.border.border-divider-dark.bg-background-secondary."focus:border-blurple-default".text-text-input.rounded-sm."p-2".transition-colors name=(name) placeholder=(placeholder) value=(value);
    }
}
